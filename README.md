...
В Git Bash для Windows версий 2.35.1-2.35.4 присутствует баг, при котором некорректно обрабатывается событие при нажатии сочетания клавиш Ctrl+C. В связи с этим может не показываться прощальное сообщение при нажатии данного сочетания клавиш. Обновите Git Bash или попробуйте запускать скрипт в другом терминале.

Добавлена проверка на версию Git с предлжением обновить, если git version 2.35.

**04 Копирование папки, 05 Сборка CSS бандла, 06 Сборка HTML страницы из компонентов и стилей**

При работе скрипта происходит вывод комментариев по текущим операциям - копирование, построение css и html.

Если это мешает проверке, запустите скрипт с флагом ```-c```, например:

```node 04-copy-directory -c off``` отключить комментирование

```node 04-copy-directory -c on``` включить комментирование
